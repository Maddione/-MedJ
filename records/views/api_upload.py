...
def upload_ocr(request):
    ...
    cat_id = request.POST.get("category_id") or request.POST.get("med_category") or request.POST.get("category")
...
def events_suggest(request):
    ...
    for ev in qs:
        try:
            ...
            items.append({"id": ev.id, "event_date": d.isoformat() if d else ""})
        except Exception:
            items.append({"id": ev.id, "event_date": ""})
