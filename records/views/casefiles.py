...
from records.models import Document, MedicalEvent, MedicalCategory, MedicalSpecialty, DocumentType, Tag
...
def casefiles(request):
    ...
    if not patient:
        ...
        return render(request, "subpages/casefiles.html", ctx)
    ...
    qs = (
        Document.objects.select_related("doc_type", "medical_event", "medical_event__specialty", "medical_event__category")
    ...
    if sel_specialties:
        qs = qs.filter(medical_event__specialty_id__in=[int(x) for x in sel_specialties if str(x).isdigit()])
    if sel_categories:
        qs = qs.filter(medical_event__category_id__in=[int(x) for x in sel_categories if str(x).isdigit()])
    ...
    ordered = sorted(groups.items(), key=lambda kv: (kv[0] == "nodate", kv[0]), reverse=True)
    grouped = [{"label": v["label"], "items": v["items"], "count": len(v["items"])} for _, v in ordered]

    base_for_counts = (
        Document.objects.select_related("medical_event", "medical_event__specialty", "medical_event__category")
        .filter(medical_event__patient_id=patient, owner=request.user)
    )
    if date_from:
        base_for_counts = base_for_counts.filter(medical_event__event_date__gte=date_from)
    if date_to:
        base_for_counts = base_for_counts.filter(medical_event__event_date__lte=date_to)
    if sel_specialties:
        base_for_counts = base_for_counts.filter(medical_event__specialty_id__in=[int(x) for x in sel_specialties if str(x).isdigit()])
    if sel_categories:
        base_for_counts = base_for_counts.filter(medical_event__category_id__in=[int(x) for x in sel_categories if str(x).isdigit()])
    ...
    categories = [
        ...
        for c in MedicalCategory.objects.filter(is_active=True).order_by("translations__name")
    ]
    ...
    tags_qs = Tag.objects.filter(documents__medical_event__patient_id=patient, documents__owner=request.user).distinct()
    tags_all = [{"id": t.id, "name": _t(t, "name")} for t in tags_qs.order_by("translations__name")]
...
def event_detail(request, pk):
    ev = get_object_or_404(MedicalEvent.objects.select_related("specialty", "category", "doc_type", "patient"), pk=pk, owner=request.user)
