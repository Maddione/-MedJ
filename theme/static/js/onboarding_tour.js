(function(){
  ...
  function onPersonal(){return /\/personalcard\/?$/.test(location.pathname)}
  function onUpload(){return /\/app\/upload\/?$/.test(location.pathname)}
  function onHistory(){return /\/app\/upload\/history\/?$/.test(location.pathname)}
  function route(){
    var o=load();if(o.done)return;
    if(onPersonal()&&o.step>1){o.step=1;save(o)}
    if(o.step===1&&onPersonal()){show("ob_personalcard");return}
    if(o.step===2&&onUpload()){show("ob_upload");return}
    if(o.step===3&&onHistory()){show("ob_history");return}
  ...
  function bindUploadLeaveGuard(){
    ...
    ["doc_type","specialty","med_category","file_input"].forEach(function(id){
      var el=document.getElementById(id);if(!el)return;el.addEventListener("change",function(){dirty=true})
    });
